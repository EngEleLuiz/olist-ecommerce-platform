...
def get_spark(local: bool = False) -> SparkSession:
    ...
    return (
        SparkSession.builder.appName("olist-bronze-ingestion")
...
def ingest_table(
    ...
    (clean.write.mode("overwrite").partitionBy("_ingestion_date").parquet(f"{dst_path}/{table}"))

    # Write rejected rows separately for investigation
    if rejected_count > 0:
        logger.warning(f"{table}: {rejected_count} rejected rows")
        rejected.write.mode("overwrite").parquet(f"{dst_path}/_rejected/{table}/{ingestion_date}")
...
def run(
    ...
    logger.info(
        f"Bronze ingestion complete — {total_valid:,} valid rows, {total_rejected} rejected"
    )
...
if __name__ == "__main__":
    if IS_GLUE:
        args = getResolvedOptions(
            sys.argv, ["JOB_NAME", "src_bucket", "dst_bucket", "ingestion_date"]
        )
