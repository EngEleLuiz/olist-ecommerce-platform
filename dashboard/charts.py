...
DARK = dict(
    ...
    legend=dict(
        bgcolor="#0f1923", bordercolor="#1e2d3d", borderwidth=1, font=dict(size=10, color="#4e6278")
    ),
...
PALETTE = [BLUE, GREEN, AMBER, RED, GOLD, PURPLE, "#80deea", "#a5d6a7", "#ffe082", "#ef9a9a"]


def _apply(fig: go.Figure, title: str = "", height: int = 380) -> go.Figure:
    fig.update_layout(
        **DARK, title=dict(text=title, font=dict(size=12, color="#cdd6e0")), height=height
    )
    return fig
...
def monthly_volume_bar(df: pd.DataFrame, title: str = "Monthly Order Volume") -> go.Figure:
    ...
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, row_heights=[0.6, 0.4], vertical_spacing=0.04
    )
    fig.add_trace(
        go.Bar(
...
def state_bar(
    df: pd.DataFrame, metric: str = "gmv", title: str = "GMV by State", top_n: int = 15
) -> go.Figure:
    agg = df.groupby("customer_state")[metric].sum().sort_values(ascending=True).tail(top_n)
...
def category_treemap(df: pd.DataFrame, title: str = "GMV by Category") -> go.Figure:
    ...
    fig.update_traces(textfont=dict(family="Space Mono", size=10), marker=dict(cornerradius=4))
...
def late_rate_by_state(df: pd.DataFrame, title: str = "Late Rate by State") -> go.Figure:
    ...
    fig.add_vline(
        ...
        line_color=BLUE,
        line_dash="dash",
        annotation_text=f"Avg {state_lr.mean():.1%}",
...
def estimated_vs_actual(df: pd.DataFrame, title: str = "Estimated vs Actual Delivery") -> go.Figure:
    ...
    fig = go.Figure(
        go.Scattergl(
            x=sample["estimated_days"],
            y=sample["actual_days"],
            mode="markers",
...
def sla_donut(df: pd.DataFrame, title: str = "Delivery SLA Breakdown") -> go.Figure:
    ...
    fig = go.Figure(
        go.Pie(
            ...
            hole=0.6,
            marker_colors=[GREEN, AMBER, RED, "#880000"],
...
def rfm_scatter(rfm: pd.DataFrame, title: str = "RFM — Recency vs Frequency") -> go.Figure:
    ...
    fig = go.Figure(
        go.Scattergl(
            x=sample["recency_days"],
            y=sample["frequency"],
            mode="markers",
            marker=dict(
                size=3,
                color=sample["total_spend"],
                colorscale="Blues",
                opacity=0.5,
...
def p_alive_histogram(predictions: pd.DataFrame, title: str = "P(Alive) Distribution") -> go.Figure:
    fig = go.Figure(
        go.Histogram(
            x=predictions["p_alive"],
            nbinsx=40,
            marker_color=GREEN,
            marker_opacity=0.8,
        )
...
def forecast_band(
    ...
    if len(fcast) > 0:
        fig.add_trace(
            go.Scatter(
                x=fcast["forecast_date"],
                y=fcast["forecast_p50"],
                name="Forecast p50",
