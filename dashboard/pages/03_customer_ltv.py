...
st.markdown("<p style='color:#4e6278;margin-top:-8px;'>BG/NBD + Gamma-Gamma lifetime value predictions</p>",
...
with col_r:
    ...
    fig = go.Figure(go.Histogram(
        x=rfm["total_spend"].clip(0, 2000), nbinsx=40,
        marker_color="#b388ff", marker_opacity=0.8,
    ))
    fig.update_layout(
        ...
        margin=dict(t=40,b=20,l=10,r=10), height=380,
    ...
    fig.add_vline(x=median_spend, line_color="#ffab40", line_dash="dash",
...
# ── BG/NBD predictions ────────────────────────────────────────────────────────
st.markdown("---")
st.markdown('<div class="section-header">BG/NBD + Gamma-Gamma Predictions</div>',
            unsafe_allow_html=True)
...
if run_model or "ltv_predictions" in st.session_state:
    ...
    st.markdown('<div class="section-header">Segment Summary</div>',
                unsafe_allow_html=True)
    ...
    st.info("Click **▶ Run LTV Model** to fit BG/NBD + Gamma-Gamma on the customer base. Takes ~30 seconds.")
