...
from dashboard.charts import (
    monthly_volume_bar, state_bar, category_treemap, review_score_bar,
)

render_sidebar()

st.markdown("# 📊 Market Overview")
st.markdown("<p style='color:#4e6278;margin-top:-8px;'>Revenue, volume and geographic breakdown</p>",
...
with col_l:
    metric = st.selectbox("State metric", ["gmv", "order_count", "payment_value"],
    ...
    st.plotly_chart(state_bar(df if metric == "gmv" else df,
                               metric=metric,
...
st.dataframe(
    region_df.rename(columns={
        "customer_region":"Region","gmv":"GMV","orders":"Orders",
...
with col_b:
    st.markdown('<div class="section-header">Top 15 Categories by GMV</div>',
                unsafe_allow_html=True)
    cat_df = (
