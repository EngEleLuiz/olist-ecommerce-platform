...
st.markdown(
    "<p style='color:#4e6278;margin-top:-8px;'>Live delivery delay prediction — enter an order and get an instant risk score</p>",
...
st.markdown(
    "<p style='color:#4e6278;'>Simulate an order and get real-time delay probability from the XGBoost model.</p>",
    unsafe_allow_html=True,
)
st.markdown("<br>", unsafe_allow_html=True)
...
with col_r:
    ...
    customer_state = st.selectbox(
        "Customer state", states, index=states.index("SP") if "SP" in states else 0
    )
...
if predict_btn:
    ...
    state_lr = (
        ...
        if customer_state in df["customer_state"].values
        else 0.1
    )
    ...
    fig_gauge = go.Figure(
        go.Indicator(
            ...
            number={
                "suffix": "%",
                "font": {"family": "Space Mono", "color": risk_color, "size": 32},
            },
            gauge={
                "axis": {
                    "range": [0, 100],
            ...
            title={
                ...
                "font": {"family": "Space Mono", "color": "#4e6278", "size": 12},
            },
        )
    )
    fig_gauge.update_layout(
        paper_bgcolor="#0f1923",
        font=dict(color="#4e6278"),
        height=300,
    ...
    # Top features
    st.markdown('<div class="section-header">Top Delay Risk Factors</div>', unsafe_allow_html=True)
