...
st.markdown(
    "<p style='color:#4e6278;margin-top:-8px;'>LightGBM 4-week-ahead forecast with p10/p50/p90 uncertainty bands</p>",
...
fig_national.add_trace(
    go.Scatter(
        x=national["week_start"],
        y=national["order_count"],
        fill="tozeroy",
        fillcolor="#40c4ff18",
...
with col_f2:
    ...
    sel_cat = st.selectbox(
        "Category", cats, index=cats.index("health_beauty") if "health_beauty" in cats else 0
    )
...
if run_forecast or "forecast_results" in st.session_state:
    ...
    if len(hist) == 0:
        ...
        if len(fcast) > 0:
            ...
            fcast_display = fcast[
                ["forecast_date", "forecast_week", "forecast_p10", "forecast_p50", "forecast_p90"]
            ].copy()
            for col in ["forecast_p10", "forecast_p50", "forecast_p90"]:
                fcast_display[col] = fcast_display[col].round(0).astype(int)
            st.dataframe(
                fcast_display.rename(
                    columns={
                        "forecast_date": "Date",
                        "forecast_week": "Week",
                        "forecast_p10": "P10",
                        "forecast_p50": "P50 (median)",
                        "forecast_p90": "P90",
                    }
                ),
                hide_index=True,
    ...
    st.markdown("---")
    st.markdown(
        '<div class="section-header">Feature Importance (p50 model)</div>', unsafe_allow_html=True
    )
